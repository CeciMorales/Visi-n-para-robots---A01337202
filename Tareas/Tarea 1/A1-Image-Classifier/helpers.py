...
def load_dataset(image_dir):
    ...
    # There are two folder called day and night
...
def standardize(image_list):
    ...
    for item in image_list:
        # Divide the image and the label so both can be process in their
        ...
        width = 1100
        height = 600
        stan_img = standardize_input(image, width, height)
        
        # Create a numerical label
        stan_label = encode(label)
        
        # Append the image, and it's one hot encoded label to the full, processed list of image data 
        standard_list.append((stan_img, stan_label))
